...
pub fn test_case(arg: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // fn_item is the test function. Includes attr and body.
    println!("===> fn_item {:?}", fn_item);
    // contains `#[allow(clippy::absurd_extreme_comparisons)]` and `#[test_case(BBBBBBBB::bbbbbbbb)]`.
    for a in fn_item.attrs.iter() {
        println!("===> fn_item.attrs has {:?}", a);
    }
    ...
    // Path { leading_colon: None, segments: [PathSegment { ident: Ident { ident: "test_case", span: #7 bytes(243..275) }, arguments: None }] }
    println!("===> legal_test_case_name {:?}", legal_test_case_name);
    for (idx, attr) in fn_item.attrs.iter().enumerate() {
        if legal_test_case_name == attr.path {
            println!("===> add test_case {:?}", attr);
            test_cases.push(attr.into_token_stream());
            attrs_to_remove.push(idx);
        }
    }
    println!("===> attrs_to_remove {:?}", attrs_to_remove);
    // like AAAAAAAA::aaaaaaaa.
    println!("===> final test_cases {:?}", test_cases);

    // Remove test_case attr itself.
    for i in attrs_to_remove.into_iter().rev() {
        fn_item.attrs.swap_remove(i);
    }
    println!("===> final fn_item.attrs {:?}", fn_item.attrs);
...
fn render_test_cases(test_cases: Vec<TokenStream2>, fn_item: ItemFn) -> TokenStream {
    ...
    for case in test_cases {
        ...
        println!("===> Handle case {:?} item {:?}", case, item);
